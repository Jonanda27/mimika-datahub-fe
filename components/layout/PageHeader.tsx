...
import { useAuthStore } from "@/src/app/store/useAuthStore"; // Pastikan path store benar
...
export default function PageHeader({ title, subtitle, withSearch, onSearch }: PageHeaderProps) {
  // Mengambil data auth dari store (Fitur dari branch HEAD)
  ...
  return (
    <div className="rounded-2xl p-5 md:p-7 mb-8 text-white flex flex-col md:flex-row justify-between items-start md:items-center shadow-md relative overflow-hidden min-h-[140px] gap-5">
      ...
      <div className="absolute inset-0 z-0">
        ...
        />
        {/* Menggunakan Opacity lebih rendah dari branch Incoming agar gambar lebih jelas */}
        <div className="absolute inset-0 bg-gradient-to-r from-[#1e61d0]/50 to-[#0b3370]/60"></div>
      </div>

      {/* 2. Content Layer (Title & Subtitle) */}
      <div className="relative z-10 w-full md:w-auto">
        <h2 className="text-xl md:text-2xl font-bold mb-1 drop-shadow-md">{title}</h2>
        <p className="text-blue-100 text-[10px] md:text-xs tracking-wide font-medium max-w-[250px] md:max-w-none drop-shadow-md">
          {subtitle}
        </p>
      </div>

      {/* 3. Action Layer (Search, Bell, & Profile) */}
      <div className="flex items-center justify-between md:justify-end gap-3 md:gap-5 relative z-10 w-full md:w-auto">

        {/* Search Input - Mengambil styling lebar dari branch Incoming */}
        {withSearch && (
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          {/* User Profile Avatar (Fitur HEAD) */}
