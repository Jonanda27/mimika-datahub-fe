...
export const ingestService = {
  async uploadProcess(data: UploadRequest): Promise<UploadResponse> {
    ...
    // ==========================================
    // Menggunakan pengecekan null/undefined agar angka 0 tetap terhitung jika ada
    if (data.district_id !== undefined && data.district_id !== null) {
      formData.append("district_id", data.district_id.toString());
    }

    // Data Files
    formData.append("file", data.file);      // File Excel/CSV/PDF
    formData.append("image", data.image);     // File Gambar Cover dari dataset
