...
export default function AkunManagementPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        // Logika Update (Password bersifat opsional)
    ...
    <div className="bg-[#f4f7fb] min-h-screen p-4 md:p-6 font-sans animate-in fade-in duration-500 text-black">
      ...
      <div className="mb-6 flex justify-between items-center">
        <div className="hidden md:block">
           <p className="text-sm text-gray-500 font-medium">Total: <span className="text-blue-600 font-bold">{users.length} User</span> Terdaftar</p>
        </div>
        <button 
          onClick={() => handleOpenModal()}
          className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-2xl flex items-center justify-center gap-2 font-bold text-sm shadow-xl shadow-blue-100 transition-all active:scale-95"
        >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-gray-50/50 border-b border-gray-100">
              <tr>
                <th className="px-6 py-4 font-bold text-gray-400 text-[11px] uppercase tracking-widest">Pengguna</th>
                <th className="px-6 py-4 font-bold text-gray-400 text-[11px] uppercase tracking-widest">Role</th>
                <th className="px-6 py-4 font-bold text-gray-400 text-[11px] uppercase tracking-widest">Status</th>
                {/* Kolom ID Akun dihapus dari sini */}
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredUsers.length > 0 ? filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-sm ${user.role === 'admin' ? 'bg-indigo-500' : 'bg-blue-400'}`}>
                  ...
                  {/* Sel data ID Akun dihapus dari sini */}
                  <td className="px-6 py-4">
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-20 text-center text-gray-300 italic">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100 flex gap-3">
                <button 
                  type="submit"
                  disabled={isLoading}
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-100 flex items-center justify-center gap-2 transition-all active:scale-95"
                >
